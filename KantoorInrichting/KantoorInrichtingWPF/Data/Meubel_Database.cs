...
namespace xmlTestding
    {
        public class MeubelDatabase
        {
            ...
            public static void ToevoegenAanDatabase(string naam, string prijs, string lengte, string breedte, string categorie, string tag, string image, string hoogte, string leverancier, string productCode)
            {
                ...
                try
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(builder.ConnectionString))
                    {
                        ...
                        //SqlCommand command = new SqlCommand(sql, connection);    
                        //{image}, {naam}, {prijs}, {lengte}, {breedte}, {tag}, {categorie}, {hoogte}
                        command.CommandText = $"INSERT INTO InventarisTest VALUES (@productcode, @leverancier, @img, @name, @prijs, @lengte, @breedte, @tag, @categorie, @hoogte) ";
            ...
            public static void UpdateCheck(string productcode)
            {
                #region test sql
                try
                {
                    ...
                    String sql = $"SELECT COUNT(*) FROM InventarisTest WHERE productcode =" + $"'{productcode}'";
                    using (SqlConnection connection = new SqlConnection(builder.ConnectionString))
                    {
                        connection.Open();
                        SqlCommand command = new SqlCommand(sql, connection);
                        ...
                        if (rowsAffected = 0)
                        {
                            ToevoegenAanDatabase();
                        }
                        else
                            MeubelDatabase.UpdateDatabase();
                    }
                }
                catch (SqlException e)
                {
